...
ENTITY LCD IS
  PORT(
    ...
    lcd_bus    : IN    STD_LOGIC_VECTOR(7 DOWNTO 0);  --data and control signals
    busy       : OUT   STD_LOGIC := '1';  --lcd controller busy/idle feedback
    rw, rs, e, pwr, bkpwr  : OUT   STD_LOGIC;  --read/write, setup/data, and enable for lcd
...
BEGIN
  ...
  PROCESS(clk,reset_n)
    ...
    VARIABLE letter_count : INTEGER := 0; --event counter for timing
  BEGIN
  if reset_n = '0' then
        state <= power_up;
        clk_count := 0;
      ...
      CASE state IS
		...
		ELSE
			rs <= '1';
            if (letter_count > 15) then
				clk_count := 0;
				state <= send;
                letter_count := letter_count + 1;
				if (letter_count = 40) then
					letter_count := 0;
				end if;
            else
				letter_count := letter_count + 1;
